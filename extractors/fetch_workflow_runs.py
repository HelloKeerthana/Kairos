...
def fetch_workflow_runs():
    ...
    response = client._request(f"https://api.github.com{endpoint}", params={"per_page": 100}).json()
    raw_runs = response.get("workflow_runs", [])

    print("RAW RESPONSE LENGTH:", len(raw_runs))

    if not raw_runs:
        return pd.DataFrame(columns=["id", "name", "status", "conclusion", "run_started_at", "updated_at", "head_branch"])
