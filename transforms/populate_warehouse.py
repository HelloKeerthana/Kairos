...
def populate():
    ...
    with engine.begin() as conn:
        for _, row in pr_df.iterrows():
            ...
            merged_at = (
                pd.to_datetime(row["merged_at"]) if pd.notna(row["merged_at"]) else None
            )
            lead_time = (
                (merged_at - created_at).total_seconds() / 3600 if merged_at else None
            )
            ...
            """),
                {
                    ...
                    "additions": (
                        int(row["additions"]) if pd.notna(row["additions"]) else None
                    ),
                    "deletions": (
                        int(row["deletions"]) if pd.notna(row["deletions"]) else None
                    ),
                    "changed_files": (
                        int(row["changed_files"])
                        if pd.notna(row["changed_files"])
                        else None
                    ),
                },
            )
