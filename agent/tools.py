...
def query_metrics_db():
    ...
    with engine.connect() as conn:
        lead_time = conn.execute(text("SELECT * FROM warehouse.v_dora_lead_time ORDER BY week DESC")).fetchall()
        deploy_freq = conn.execute(text("SELECT * FROM warehouse.v_dora_deployment_frequency ORDER BY week DESC")).fetchall()
        failure_rate = conn.execute(text("SELECT * FROM warehouse.v_dora_change_failure_rate ORDER BY week DESC")).fetchall()
        anomalies = conn.execute(text("SELECT * FROM warehouse.anomalies ORDER BY detected_at DESC")).fetchall()
